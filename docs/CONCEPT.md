...
VibeBox uses one global store at `~/.vibebox` by default, overrideable with `VIBEBOX_HOME`. It never creates project-local `.vibebox/`, workspace-local memory snapshots, copied stores, pointer files, or hidden metadata in a work repository. Sandboxed hosts can require approval to read or write that store because it is outside the workspace. `pretask` and `context` are read-only retrieval commands and do not register a project; `init`, `aftertask`, and `capture` register the current AI working directory when write access is available. That current working directory is treated as a project workspace by default, whether it is a framework repository, static HTML/PHP folder, JSON-only app folder, documentation folder, or plain folder. Git remotes and package metadata are identity hints, not admission requirements. User home, the global store itself, drive roots, plugin caches, `.codex`, `.agents`, `node_modules`, and system temp roots stay outside `registry/projects.json`.
