...
In Codex App or other sandboxed hosts, the global store may require explicit access because it is outside the workspace. VS Code or Codex CLI sessions may already have user-home access, while Codex App can require separate approval for `~/.vibebox`. `pretask` and `context` are read-only memory retrieval for repository files but still read the global store; they do not create project registry entries. `init`, `aftertask`, and `capture` can register the current project when write access is available. If read approval is denied, report VibeBox guidance unavailable. If aftertask write approval is denied, report capture unavailable and state that project registration, active memory, and wiki updates did not happen. See `adapters/codex/README.md` for the Codex sandbox and approval guidance.
